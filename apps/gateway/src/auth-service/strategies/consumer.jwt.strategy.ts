...
export class ConsumerJwtStrategy extends PassportStrategy(
  ...
  constructor(
    ...
    super({
      ...
      secretOrKey:
        (process.env.CONSUMER_ACCESS_JWT_SECRET as string) || 'secret',
    });
  }
  ...
  async validate(payload: { id: string }): Promise<Omit<User, 'password'>> {
    try {
      const user = await this.rpcClient.send<Omit<User, 'password'>>(
        this.authClient,
        { cmd: AUTH_VALIDATE_TOKEN_METHOD },
        { userId: payload.id },
      );
      ...
      this.errorHandler.handleRpcError(error, AUTH_VALIDATE_TOKEN_METHOD);
    }
  }
}
